...
from util.UtilClass import LayerNorm
from util.UtilClass import PositionwiseFeedForward
from util.MultiHeadedAttn import MultiHeadedAttention
...
class HierarchicalContext(nn.Module):
    ...
    def __init__(self, size, dropout,
        ...
        self.layer_norm_query_word = LayerNorm(size)
        self.layer_norm_query_sent = LayerNorm(size)
        self.layer_norm_word = LayerNorm(size)
        self.dropout = nn.Dropout(dropout)
        self.sent_attn = MultiHeadedAttention(head_count, size, dropout=dropout)
        self.word_attn = MultiHeadedAttention(head_count, size, dropout=dropout)

        self.linear = nn.Linear(2 * size, size)
        self.sigmoid = nn.Sigmoid()

        self.layer_norm_sent = LayerNorm(size)
